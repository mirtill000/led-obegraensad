...
void AmbientMode::begin(uint8_t animation) {
  ...
  memset(starSpan_, 0, sizeof(starSpan_));
...
void AmbientMode::fire() {
  ...
  for (int y = 0; y < ROWS; y++) {
    for (int x = 0; x < COLS; x++) {
      const int level = (heat_[y][x] - 50) * 255 / 170;
      display.setLevel(x, y, level < 0 ? 0 : level > 255 ? 255 : level);
    }
  }
}
...
void AmbientMode::stars() {
  ...
  if (esp_random() % 3 == 0) {
    ...
    if (starLife_[y][x] == 0) starLife_[y][x] = starSpan_[y][x] = 20 + esp_random() % 40;
  }
  display.clear();
  for (int y = 0; y < ROWS; y++) {
    for (int x = 0; x < COLS; x++) {
      const int life = starLife_[y][x];
      if (life == 0) continue;
      const int age = starSpan_[y][x] - life;
...
void AmbientMode::waves(uint32_t now) {
  ...
  for (int y = 0; y < ROWS; y++) {
    for (int x = 0; x < COLS; x++) {
      ...
      const float level = (v + 0.4f) / 2.4f;  // v is in -3..3
      display.setLevel(x, y, level <= 0 ? 0 : level >= 1 ? 255 : (uint8_t)(level * 255));
    }
  }
}

// A soft-edged circle that grows (breathe in, 4 s) and shrinks (breathe
// out, 6 s).
void AmbientMode::breath(uint32_t now) {
  ...
  for (int y = 0; y < ROWS; y++) {
    for (int x = 0; x < COLS; x++) {
      ...
      const float edge = radius - sqrtf(dx * dx + dy * dy) + 0.5f;  // >1 inside, <0 outside
      display.setLevel(x, y, edge <= 0 ? 0 : edge >= 1 ? 255 : (uint8_t)(edge * 255));
    }
  }
}
