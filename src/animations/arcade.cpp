// Arcade games: Pong, Breakout, Flappy Bird, Space Invaders and 2048. In
...
// ---------------------------------------------------------------------------
// 2048 on a 4x4 board: each tile is a 3x3 square whose brightness shows
// its value (2 faint ... 2048 brightest).
class Game2048 : public ArcadeGame {
 public:
  const char *id() const override { return "2048"; }
  const char *name() const override { return "2048"; }
  uint16_t frameMs() const override { return 50; }

  void start() override {
    memset(board_, 0, sizeof(board_));
    memset(fresh_, 0, sizeof(fresh_));
    points_ = 0;
    addTile();
    addTile();
    lastMove_ = 0;
  ...
  void input(char key) override {
    const int dir = key == 'L' ? 0 : key == 'R' ? 1 : key == 'U' ? 2 : key == 'D' ? 3 : -1;
    if (dir >= 0) play(dir);
  }

 protected:
  void tick(uint32_t now) override {
    if (demo_ && now - lastMove_ >= 400) {
      lastMove_ = now;
      play(bestMove());
    }
    for (int i = 0; i < 16; i++) {
      if (fresh_[i] > 0) fresh_[i]--;
    }
    draw();
  }

 private:
  typedef uint8_t Board[16];  // exponents: 1 = 2, 2 = 4, ... 0 = empty

  // Slides the board in direction 0-3 (left, right, up, down); returns
  // points scored, or -1 if nothing moved.
  static int slide(Board b, int dir) {
    int points = 0;
    bool moved = false;
    for (int line = 0; line < 4; line++) {
      int idx[4];
      for (int k = 0; k < 4; k++) {
        // k-th cell in the direction of travel, starting from the far edge.
        const int j = (dir == 1 || dir == 3) ? 3 - k : k;
        idx[k] = dir <= 1 ? line * 4 + j : j * 4 + line;
      }
      uint8_t out[4] = {0};
      int n = 0;
      bool merged = false;
      for (int k = 0; k < 4; k++) {
        const uint8_t v = b[idx[k]];
        if (!v) continue;
        if (n > 0 && out[n - 1] == v && !merged) {
          out[n - 1]++;
          points += 1 << out[n - 1];
          merged = true;
        } else {
  ...
  void addTile() {
    int empty[16], n = 0;
    for (int i = 0; i < 16; i++) {
      if (!board_[i]) empty[n++] = i;
    }
    if (!n) return;
    const int i = empty[esp_random() % n];
    board_[i] = esp_random() % 10 == 0 ? 2 : 1;
    fresh_[i] = 6;  // fades in
  }

  void play(int dir) {
    const int points = slide(board_, dir);
    if (points < 0) return;
    points_ += points;
    addTile();
    bool canMove = false;
    for (int d = 0; d < 4 && !canMove; d++) {
      Board t;
      memcpy(t, board_, sizeof(t));
      canMove = slide(t, d) >= 0;
    }
    if (!canMove) gameOver(points_);
  }

  // Board quality: empty cells, big tiles in a corner, smooth rows/columns.
  static float score(const Board b) {
    float s = 0;
    int empty = 0;
    for (int i = 0; i < 16; i++) empty += !b[i];
    s += empty * 2.7f;
    for (int r = 0; r < 4; r++) {
      for (int c = 0; c < 3; c++) {
        s -= fabsf((float)b[r * 4 + c] - b[r * 4 + c + 1]) * 0.1f;
        s -= fabsf((float)b[c * 4 + r] - b[(c + 1) * 4 + r]) * 0.1f;
        if (b[r * 4 + c] >= b[r * 4 + c + 1]) s += 0.2f;  // decreasing to the right
        if (b[c * 4 + r] >= b[(c + 1) * 4 + r]) s += 0.2f;  // and downwards
      }
    }
    s += b[0] * 1.0f;  // biggest tile in the top-left corner
    return s;
  }

  // Computer player: each move, then the average over where the next tile
  // may appear, then the best reply.
  int bestMove() const {
    int best = 0;
    ...
    for (int d = 0; d < 4; d++) {
      ...
      for (int i = 0; i < 16; i++) {
        ...
        for (int d2 = 0; d2 < 4; d2++) {
          Board v;
          memcpy(v, u, sizeof(v));
          if (slide(v, d2) >= 0) reply = fmaxf(reply, score(v));
        }
        total += reply;
        cases++;
      }
      const float value = (cases ? total / cases : score(t)) + pts * 0.01f;
      if (value > bestValue) {
        bestValue = value;
        best = d;
      }
  ...
  void draw() {
    ...
    for (int i = 0; i < 16; i++) {
      if (!board_[i]) {
        display.setLevel((i % 4) * 4 + 1, (i / 4) * 4 + 1, 12);  // empty cell: a faint dot
        continue;
      }
      int level = 25 + (board_[i] - 1) * 23;  // 2 -> 25 ... 2048 -> 255
      if (level > 255) level = 255;
      level = level * (6 - fresh_[i]) / 6;
      for (int dy = 0; dy < 3; dy++) {
        for (int dx = 0; dx < 3; dx++) display.setLevel((i % 4) * 4 + dx, (i / 4) * 4 + dy, level);
      }
    }
  }
...
extern Animation *const game2048Animation = &game2048;
